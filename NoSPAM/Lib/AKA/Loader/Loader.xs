...
#define CRYPT_MAGIC_1	0xfe
#define CRYPT_MAGIC_2	0x01

#define HEADERSIZE	2
#define BLOCKSIZE	22
/* 22 * 512 = 11KB, reduce read num.*/
#define BLOCKNUM	512
...
static unsigned XOR [BLOCKSIZE] = {'o', 'k', 'b', 'o','y',
					'&',
				   'z', 'i', 'x', 'i', 'a',
					'@',
				   '2', '0', '0', '4', '-', '0', '3', '-', '0', '7' } ;
...
Decrypt(SV *in_sv, SV *out_sv)
{
	...
	out_buffer = (unsigned char *) SvPVX(out_sv) ;
        ...
        /* XOR */
        for (i = 0 ; i < size ; ++i) 
            out_buffer[i] = (unsigned char)( XOR[i%BLOCKSIZE] ^ in_buffer[i] ) ;
...
static I32
Loader_decrypt(pTHX_ int idx, SV *buf_sv, int maxlen)
{
	...
	/* been applied on the same line as the use AKA::Loader	*/
...
#ifdef FDEBUG
    if (fdebug)
	warn("**** In Loader_decrypt - maxlen = %d, len buf = %d idx = %d\n", 
	...
	/* read from the file into the encrypt buffer */
 	if ( (n = ReadBlock(idx+1, ENCRYPT_SV(my_sv), BLOCKSIZE*BLOCKNUM)) <= 0)
	{
...
#ifdef FDEBUG
	if (fdebug)
	    warn("  AKA_Loader(%d): sub-Loader returned %d: '%.999s'",
...
MODULE = AKA::Loader	PACKAGE = AKA::Loader
...
import(module)
    ...
    PPCODE:
    {
        ...
        SV * sv = newSV(BLOCKSIZE*BLOCKNUM) ;

	/* make sure the Perl debugger isn't enabled */
	if( PL_perldb )
	    croak("debugger disabled") ;

        filter_add(Loader_decrypt, sv) ;
	FIRST_TIME(sv) = TRUE ;

        ENCRYPT_GV(sv) = (GV*) newSV(BLOCKSIZE*BLOCKNUM) ;
