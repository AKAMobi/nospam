...
static void do_login(const char *, const char *, int);

void sqpcp_login(const char *userid, const char *password)
{
	do_login(userid, password, 0);
}

static void do_login(const char *userid, const char *password, int showerr)
{
	struct PCP *pcp;
	...
	if (sqpcp_has_groupware())	/* Groupware mode, login to server */
	{
		...
		{
			printf("%s<blockquote><pre class=\"small-error\">",
...
static int need_refresh()
{
	...
	if (fp)
	{
		...
		if (fgets(buffer, sizeof(buffer), fp) &&
		    sscanf(buffer, "%lu\t%lu", &a, &b) == 2)
		{
			time_t now;

			time(&now);

			if (now < (time_t)b)	/* Event expired */
			{
		       ...
		       time_t start, time_t end)
{
	...
	*n_recs=n;
	...
	if (pcp_retr(pcp, &r))
	{
		syslog2(LOG_NOTICE, "pcp_retr: error: %s", pcp_errmsg(pcp));
		free(event_ids);
		destroycache(*recs, *n_recs);
		return (-1);
	}
...
void sqpcp_eventstart()
{
	...
	while (rfc822hdr_read(&h, oldfp, NULL, 0) == 0)
	{
		...
		else if (strcasecmp(h.header, "from") == 0 && h.value)
		{
			...
			if (from_buf)
				free(from_buf);
			from_buf=rfc2047_decode_enhanced
				(h.value, sqwebmail_content_charset);
			if (from_buf)
		...
		else if (strcasecmp(h.header, "subject") == 0 && h.value)
		{
			...
			if (subj_buf)
				free(subj_buf);
			subj_buf=rfc2047_decode_enhanced
				(h.value, sqwebmail_content_charset);
			if (subj_buf)
...
static void print_event_subject(char *flags, const char *subject, unsigned w)
{
	...
	p=rfc2047_decode_enhanced(subject ? subject:"",
...
static FILE *openoldfp(const char *p, unsigned long *prev_size)
{
	...
	filename=maildir_find(DRAFTS, p);
	if (!filename)
		return (NULL);
...
static void show_conflict_error(struct PCP *pcp)
{
	...
	if (n == 0 || pcp_retr(pcp, &r) == 0)
	{
		...
		for (p=conflict_list; p; p=p->next)
		{
			...
			printf("<tr><td width=30>&nbsp;</td><td><tt>");
			if (pcp_fmttimerange(buffer, sizeof(buffer),
					     p->start,
					     p->end) == 0)
			{
			...
			printf("</tt></td><td width=30>&nbsp;</td><td width=\"100%%\"><tt>");
...
void sqpcp_newevent()
{
	...
	if (p && *p)
	{
		newdraftfd=maildir_recreatemsg(DRAFTS, p, &draftfilename);
	}
	else
	{
	...
	if (errflag)
	{
		maildir_deletenewmsg(newdraftfd, DRAFTS, draftfilename);
	}
	else if (maildir_closemsg(newdraftfd, DRAFTS, draftfilename, 1,
				  prev_size))
	{
		printf("%s", getarg("QUOTAERR"));
	}
...
static char *savedraft()
{
	...
	filename=p && *p ? maildir_find(DRAFTS, p):NULL;
...
void sqpcp_save()
{
	...
	sentmsg=newmsg_createsentmsg(msg, &isgpgerr, 0);

	/* Immediately remove the formatted event text from the sent folder */

	if (sentmsg)
	{
		p=maildir_find(SENT, sentmsg);
		free(sentmsg);
		sentmsg=p;
	}
				   ...
				   void *voidarg)
{
	    ...
	    (e->userid=strdup(userid)) == NULL)
	{
		if (e)
			free(e);
		syslog(LOG_CRIT, "sqwebmail: out of memory.");
		return;
	}
				   ...
				   unsigned tn)
{
	...
	{
		syslog(LOG_CRIT, "sqwebmail: seek failed: %m");
		return;
	}
	...
	if ((tmpfp=open_langform(sqwebmail_content_language, template, 0))
	    == NULL)
	{
		syslog2(LOG_CRIT, "sqwebmail: %s: %m", template);
		return;
	}
	...
	{
		fclose(tmpfp);
		syslog(LOG_CRIT, "sqwebmail: pipe: %m");
		return;
	}
	...
	if (p < 0)
	{
		...
		close(pipefd[1]);
		fclose(tmpfp);
		syslog(LOG_CRIT, "sqwebmail: fork: %m");
		return;
	}
	...
	if (p == 0)
	{
		...
		syslog(LOG_CRIT, "sqwebmail: exec " SENDITSH ": %m");
		exit(1);
	}
	close(pipefd[0]);
	...
	if (!WIFEXITED(waitstat) || WEXITSTATUS(waitstat))
		syslog(LOG_CRIT, "sqwebmail: event notify mail failed");
}
...
void sqpcp_daily_view()
{
	...
	{
		printf("<font class=\"error\">%s</font>", strerror(errno));
		return;
	}
...
void sqpcp_displayevent()
{
	...
	{
		printf("<tr valign=top><th align=left>%s</th></td><td>",
		       getarg("PARTICIPANTS"));
...
int sqpcp_eventedit()
{
	...
	else if (maildir_closemsg(newdraftfd, DRAFTS, draftfilename, 1, 0))
	{
		cgi_put("error", "quota");
	}
	else
