...
struct rfc2045 *gpgmime_is_multipart_signed(const struct rfc2045 *q)
{
	struct rfc2045 *p;
...
struct rfc2045 *gpgmime_is_multipart_encrypted(const struct rfc2045 *q)
{
	struct rfc2045 *p;
...
int gpgmime_has_mimegpg(const struct rfc2045 *q)
{
	if (gpgmime_is_multipart_signed(q) ||
	    gpgmime_is_multipart_encrypted(q))
		return (1);
	...
	for (q=q->firstpart; q; q=q->next)
	{
		if (q->isdummy)
			continue;
		if (gpgmime_has_mimegpg(q))
			return (1);
	}
	return (0);
}

int gpgmime_is_decoded(const struct rfc2045 *q, int *retcode)
{
	const char *p;
...
struct rfc2045 *gpgmime_decoded_content(const struct rfc2045 *q)
{
	for (q=q->firstpart; q; q=q->next)
	{
		if (q->isdummy)
...
struct rfc2045 *gpgmime_signed_content(const struct rfc2045 *p)
{
	struct rfc2045 *q;
