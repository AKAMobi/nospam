...
static int list_msg_rfc822(struct rfc2045 *rfc, FILE *fp)
{
	...
	{
		char *hh=rfc2047_decode_enhanced(h.value, charset);

		if (!hh)
		{
			perror("rfc2047_decode_enhanced");
			exit(1);
		}
		printf("%s: %s\n", h.header, hh);
		free(hh);
...
static int list_msg_textplain(struct rfc2045 *rfc, FILE *fp)
{
	...
	int rc;
        ...
        {
		printf(gettext("    (%s charset follows)\n\n"),
		       mime_charset);
	}

	rc=rfc2045_decodetextmimesection(fileno(fp),
					 rfc,
					 charset,
