...
static void savetokens()
{
	...
	syslog(LOG_CRIT, "pcpd: cannot save authentication tokens");
}
	...
	if (fp)
	{
		    ...
		    fgets(buf2, sizeof(buf2), fp))
		{
			    ...
			    stat_buf.st_mtime >= expire_time - TIMEOUT*2)
			{
				...
				fclose(fp);
				syslog(LOG_NOTICE, "pcpd: restored saved authentication tokens");
				return;
			}
		}
		fclose(fp);
...
int authtoken_verify(const char *userid, const char *token, time_t *when)
{
	...
	*when=tval;
	...
	syslog(LOG_CRIT, "pcpd: authtoken_create() failed.");
	return (-1);
}
