...
function doConfig(){
	...
	$handle=fopen("/var/qmail/control/black_addr","w");	
	if (!$handle) {
		echo "错误，无法保存设置！<br>";
		return false;
	}
...
if ( (isset($_REQUEST['doConfig']) && doConfig()) ){
	...
	if ( ! file_exists("/var/qmail/control/black_addr") ){
		fclose( fopen("/var/qmail/control/black_addr","w") );
	}

	$handle = fopen ("/var/qmail/control/black_addr", "r");
	$list="";
	while (!feof ($handle)) {
	...
	?>
		...
		<td colspan="2" class=title><b>设置地址黑名单列表</b></td>
		</tr>
		<tr>
		<td colspan="2">地址黑名单：
		</td>
