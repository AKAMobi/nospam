...
function doConfig(){
	...
	$handle=fopen("/var/qmail/control/white_ip","w");	
	if (!$handle) {
		echo "错误，无法保存设置！<br>";
		return false;
	}
...
if ( (isset($_REQUEST['doConfig']) && doConfig()) ){
	...
	if ( ! file_exists("/var/qmail/control/white_ip") ){
		fclose( fopen("/var/qmail/control/white_ip","w") );
	}

	$handle = fopen ("/var/qmail/control/white_ip", "r");
	$list="";
	while (!feof ($handle)) {
	...
	?>
		...
		<td colspan="2" class=title><b>设置IP白名单列表</b></td>
		</tr>
		<tr>
		<td colspan="2">IP白名单：
		</td>
